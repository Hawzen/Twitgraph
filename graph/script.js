...
const tau = 2 * Math.PI;
const maxCluster = Math.max.apply(
  Math,
  Object.values(data.variables.clusterSizes)
);
...
if (c.includes("a"))
  // rgba(a, b, c, d) or rgb(a, b, c)
  coloring.clusterActive = c
    .slice(5, -1)
    .split(",")
    .map((x) => parseFloat(x));
else
...
document.getElementById("container").style["background-color"] =
  coloring.background;
document.getElementById("miniMap").style["background-color"] =
  coloring.miniMapBackground;
...
const addNode = function (cluster) {
  ...
  if (data.variables.clusters.some((x) => cluster == x)) {
    // Get name from cluster's children, assign as label
    loop1: for (node in data.nodes)
    ...
    size =
      (data.variables.clusterSizes[cluster] / maxCluster) *
...
if (data.variables.clusterEdges.length > 0)
    ...
    for (j in data.variables.clusterEdges[i]) {
      ...
      if (data.variables.clusterEdges[i][j])
        g.edges.push({
          id:
            "e_c" +
            data.variables.clusters[i] +
            "_c" +
            data.variables.clusters[j],
  ...
  const getClusterEdge = function (cluster) {
    while (true) {
      ...
      if (g.nodes.some((x) => x.id == "c" + cluster)) return cluster;
    }
  };
      ...
      if (prev != null) {
        if (!g.edges.some((x) => x.id === "e_c" + cluster + "_c" + prev))
          g.edges.push({
            ...
            color: idColor(
              "c" + cluster,
              coloring.clusterActive,
              (a = 1 / ("c" + cluster).length)
            ),
            type: config.edges.clusterEdges,
          });
      } else {
...
function displayDetails(id, append = false) {
  ...
  if (id.includes("c"))
    if (!data.variables.clusters.includes(id.slice(1)))
      // If clicked node is a cluster
      // if cluster doesn't have nodes then ignore it
      return;
    else result = clusterDetails(id);
  else result = profileDetails(data.nodes[id]);

  if (result == -1) return;

  if (stack.length == constants.maxProfiles && append) return; // if at max and trying to append
...
function addbox(append, result) {
  ...
  } else infoBox.innerHTML = result;
}

function profileDetails(node) {
  if (stack.includes(node.json.id_str)) return -1;
  // if node already in box
  else stack.push(node.json.id_str);
...
function clusterDetails(cluster) {
  ...
  for (node in data.nodes) {
    ...
    if (cluster.slice(1) != node.cluster) continue;
  ...
  description = imgs;
  return (
    '<div style="background-color: rgb(20, 20, 20, 0.1); text-align:center;">' +
    header +
    imgs +
    "</div>"
  );
...
function markEdges(id, onColor, offColor, ctrlKey = false) {
  ...
  for (let edge in edges) {
    ...
    } else if (ctrlKey && edges[edge].active) {
    } // Skip when user holds ctrlKey and edge already active
    else {
...
document.getElementById("visibility").onclick = function () {
  ...
  for (let key in nodes) {
    if (nodes[key].id.includes("c")) continue;
    nodes[key].hidden = !b;
  }
  s.refresh();
};
...
for (node in nodes) nodes[node].active = false;
...
s.bind("overNode", function (e) {
  ...
  if (!keepDetails) displayDetails(nodegraph.id);
});

s.bind("outNode", function (e) {
  /* Remove details put by event overNode when cursor out of node */
  if (!keepDetails) clearDetails();
});
...
s.bind("doubleClickNode", function (e) {
  ...
  markEdges(
    nodegraph.id,
    coloring.activeEdge,
    coloring.inactiveEdge,
    e.data.captor.ctrlKey
  );
  markNodes(
    nodegraph.id,
    coloring.active,
...
function searchChange(e) {
  ...
  s.graph.nodes().forEach(function (n) {
    if (n.label == value) {
      ...
      s.cameras.main.goTo(coordinate);
      console.log(n);
...
function idColor(cluster, c, a = undefined) {
  ...
  if (a === undefined)
    return `rgba(${c[0]}, ${(c[1] * sum) / 10}, ${(c[2] * sum) / 3}, ${
      c.length > 3 ? c[3] : 1
    })`;
  else return `rgba(${c[0]}, ${(c[1] * sum) / 10}, ${(c[2] * sum) / 3}, ${a})`;
}
